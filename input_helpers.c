...
char *get_args(char *line, int *exec_ret)
{
	...
	line[read - 1] = '\0';
	variable_replacement(&line, exec_ret);
...
/**
 * parse_args - Partitions operators from commands and calls them.
...
int parse_args(char **args, char **front, int *exec_ret)
{
	...
	for (index = 0; args[index]; index++)
	{
		if (_strncmp(args[index], "||", 2) == 0)
		{
			...
			ret = run_args(args, front, exec_ret);
			if (*exec_ret != 0)
			{
				args = &args[++index];
				index = 0;
...
int handle_args(int *exec_ret)
{
	...
	for (index = 0; args[index]; index++)
	{
		if (_strncmp(args[index], ";", 1) == 0)
		{
			free(args[index]);
			args[index] = NULL;
			ret = parse_args(args, front, exec_ret);
			args = &args[++index];
			index = 0;
		}
	}
